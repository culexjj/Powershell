...
#We need to replace here <"domain"> (removing <>) with the domain name
#We need to replace here <domain\admin> (removing <>) with a domain name & user with privileges to do this action
Add-Computer -ComputerName $ComputerName -DomainName <"domain"> -Credential <domain\admin> -Restart -Force